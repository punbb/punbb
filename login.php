...
else if ($action == 'out')
{
	...
	redirect(link('index'), __('Logout redirect', 'login'));
}
...
else if ($action == 'forget' || $action == 'forget_2')
{
	...
	if (isset($_POST['form_sent']))
	{
		...
		if (isset($_POST['cancel']))
			redirect(link('index'), __('New password cancel redirect', 'login'));
		...
		if (empty($errors))
		{
			...
			if (!empty($users_with_email))
			{
				...
				foreach ($users_with_email as $cur_hit)
				{
					...
					$cur_mail_message = str_replace('<activation_url>', str_replace('&amp;', '&', link('change_password_key', array($cur_hit['id'], $new_password_key))), $cur_mail_message);
