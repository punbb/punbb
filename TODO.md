...
- как различать  для расширения что грузить __('Link to', 'extension1'),  проверять $domain в списке расширений или лучше добавить префикс __('Link to', 'ext.extension1') ? - новый "домен" 'extension1' регистрируется каким-то образом (путь к файлу для загрузки добавляется) через расширение типа "языковой пакет"
- __fmt('const', [arg1, arg2, arg3], $domain, $language) - работает как sprintf(__(...), arg1, arg2,)

### шаблоны

стандартный view, helper - выводит данные
возможность переопределить (может реализовать на классах с DI)
