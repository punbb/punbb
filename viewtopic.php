...
// If action=new, we redirect to the first new post (if any)
	...
	if (!user()->is_guest) {
		...
		if ($first_new_post_id) {
			header('Location: '.str_replace('&amp;', '&', link('post', $first_new_post_id)));
			exit;
		}
	}

	header('Location: '.str_replace('&amp;', '&', link('topic_last_post', $id)));
	exit;
}
// If action=last, we redirect to the last post
	...
	if ($last_post_id) {
		header('Location: '.str_replace('&amp;', '&', link('post', $last_post_id)));
		exit;
	}
}
...
if ($forum_page['is_admmod']) {
	$forum_page['main_foot_options'] = array(
		'move' => '<span class="first-item"><a class="mod-option" href="'.link('move', array($cur_topic['forum_id'], $id)).'">'.
			__('Move', 'topic') . '</a></span>',
		'delete' => '<span><a class="mod-option" href="'.link('delete', $cur_topic['first_post_id']).'">'.
			__('Delete topic', 'topic') . '</a></span>',
		'close' => (($cur_topic['closed'] == '1') ? '<span><a class="mod-option" href="'.link('open', array($cur_topic['forum_id'], $id, generate_form_token('open'.$id))).'">'.
			__('Open', 'topic') . '</a></span>' : '<span><a class="mod-option" href="'.link('close', array($cur_topic['forum_id'], $id, generate_form_token('close'.$id))).'">'.
			__('Close', 'topic') . '</a></span>'),
		'sticky' => (($cur_topic['sticky'] == '1') ? '<span><a class="mod-option" href="'.link('unstick', array($cur_topic['forum_id'], $id, generate_form_token('unstick'.$id))).'">'.
			__('Unstick', 'topic') . '</a></span>' : '<span><a class="mod-option" href="'.link('stick', array($cur_topic['forum_id'], $id, generate_form_token('stick'.$id))).'">'.
