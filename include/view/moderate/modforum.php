...
($hook = get_hook('mr_topic_actions_output_start')) ? eval($hook) : null;
	...
	include template()->view('moderate/topics_start');
	...
	while ($cur_topic = db()->fetch_assoc($result))
	{
		...
		if ($cur_topic['moved_to'] != null)
		{
			...
			$forum_page['item_title']['link'] = '<span class="item-status"><em class="moved">'.
				sprintf(__('Item status', 'forum'), __('Moved', 'forum')).'</em></span> <a href="'.link('topic', array($cur_topic['moved_to'], sef_friendly($cur_topic['subject']))).'">'.forum_htmlencode($cur_topic['subject']).'</a>';
		...
		else
		{
			...
			$forum_page['item_title']['link'] = '<a href="'.link('topic', array($cur_topic['id'], sef_friendly($cur_topic['subject']))).'">'.forum_htmlencode($cur_topic['subject']).'</a>';
			...
			{
				$forum_page['item_nav']['new'] = '<em class="item-newposts"><a href="'.link('topic_new_posts', array($cur_topic['id'], sef_friendly($cur_topic['subject']))).'">'.
			...
			$forum_page['item_body']['info']['lastpost'] = '<li class="info-lastpost"><span class="label">'.
				__('Last post', 'forum') . '</span> <strong><a href="'.link('post', $cur_topic['last_post_id']).'">'.format_time($cur_topic['last_post']).'</a></strong> <cite>'.
		...
		include template()->view('moderate/topic');
	}

	include template()->view('moderate/topics_end');
