...
$links['index'] = '<li id="navindex"'.((FORUM_PAGE == 'index') ? ' class="isactive"' : '').'><a href="'.link('index').'">'.
	__('Index') . '</a></li>';

if (user()->g_read_board == '1' && user()->g_view_users == '1')
	$links['userlist'] = '<li id="navuserlist"'.((FORUM_PAGE == 'userlist') ? ' class="isactive"' : '').'><a href="'.link('users').'">'.
	__('User list') . '</a></li>';

if (config()->o_rules == '1' && (!user()->is_guest || user()->g_read_board == '1' ||
		config()->o_regs_allow == '1'))
	$links['rules'] = '<li id="navrules"'.((FORUM_PAGE == 'rules') ? ' class="isactive"' : '').'><a href="'.link('rules').'">'.
	__('Rules') . '</a></li>';
...
if (user()->is_guest) {
	...
	$links['register'] = '<li id="navregister"'.((FORUM_PAGE == 'register') ? ' class="isactive"' : '').'><a href="'.link('register').'">'.
		__('Register') . '</a></li>';
	$links['login'] = '<li id="navlogin"'.((FORUM_PAGE == 'login') ? ' class="isactive"' : '').'><a href="'.link('login').'">'.
		__('Login') . '</a></li>';
}
else {
	if (!user()->is_admmod) {
