...
?>
	...
	<form id="aus-find-user-results-form" class="frm-form" method="post" accept-charset="utf-8" action="<?php echo link('admin_users') ?>?action=modify_users">
	...
	if ($forum_page['num_users'] > 0)
	{
		...
		while ($user_data = db()->fetch_assoc($result))
		{
			...
			$forum_page['table_row']['actions'] = '<td class="tc'.count($forum_page['table_row']).'"><span><a href="'.link('admin_users').'?ip_stats='.$user_data['id'].'">'.
				__('View IP stats', 'admin_users').'</a></span> <span><a href="'.link('search_user_posts', $user_data['id']).'">'.
