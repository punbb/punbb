...
?>
	...
	<div class="main-content main-frm">
		<form class="frm-form" method="post" accept-charset="utf-8" action="<?php echo link('admin_censoring') ?>?action=foo">
			<div class="hidden">
				<input type="hidden" name="csrf_token" value="<?php echo generate_form_token(link('admin_censoring').'?action=foo') ?>" />
			</div>
			<div class="ct-box" id="info-censored-intro">
				<p><?= __('Add censored word intro', 'admin_censoring'); if (user()->g_id == FORUM_ADMIN) printf(' '.
					__('Add censored word extra', 'admin_censoring'), '<a class="nowrap" href="'.link('admin_settings_features').'">'.
