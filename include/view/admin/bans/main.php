...
<?php
	...
	while ($cur_ban = db()->fetch_assoc($result))
	{
		...
		$forum_page['ban_creator'] = ($cur_ban['ban_creator_username'] != '') ? '<a href="'.link('user', $cur_ban['ban_creator']).'">'.forum_htmlencode($cur_ban['ban_creator_username']).'</a>' :
			...
			<div class="ct-set set<?php echo ++$forum_page['item_num'] ?>">
				<div class="ct-box">
					<div class="ct-legend">
						...
						<p><?php printf(__('Edit or remove', 'admin_bans'), '<a href="'.link('admin_bans').'&amp;edit_ban='.$cur_ban['id'].'">'.
							__('Edit ban', 'admin_bans') . '</a>', '<a href="'.link('admin_bans').'&amp;del_ban='.$cur_ban['id'].'&amp;csrf_token='.generate_form_token('del_ban'.$cur_ban['id']).'">'.
