...
?>
	...
	<div class="main-content main-frm">
		<form id="arp-new-report-form" class="frm-form" method="post" accept-charset="utf-8" action="<?php echo link('admin_reports') ?>?action=zap">
			<div class="hidden">
				<input type="hidden" name="csrf_token" value="<?php echo generate_form_token(link('admin_reports').'?action=zap') ?>" />
			</div>
<?php
	...
	{
		$reporter = ($cur_report['reporter'] != '') ? '<a href="'.link('user', $cur_report['reported_by']).'">'.forum_htmlencode($cur_report['reporter']).'</a>' : __('Deleted user', 'admin_reports');
...
<?php
	...
	foreach ($zapped_reports as $cur_report)
	{
		...
		$message = str_replace("\n", '<br />', forum_htmlencode($cur_report['message']));
		$post_id = ($cur_report['pid'] != '') ? '<a href="'.link('post', $cur_report['pid']).'">'.sprintf(__('Post', 'admin_reports'), $cur_report['pid']).'</a>' : __('Deleted post', 'admin_reports');
		$zapped_by = ($cur_report['zapped_by'] != '') ? '<a href="'.link('user', $cur_report['zapped_by_id']).'">'.forum_htmlencode($cur_report['zapped_by']).'</a>' : __('Deleted user', 'admin_reports');
