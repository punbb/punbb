...
else if ($action == 'markread')
{
	...
	redirect(link('index'), __('Mark read redirect', 'misc'));
}
...
else if ($action == 'markforumread')
{
	...
	redirect(link('forum', array($fid, sef_friendly($forum_name))),
...
else if ($action == 'opensearch')
{
	...
	echo "\t".'<moz:SearchForm>'.link('search').'</moz:SearchForm>'."\n";
	...
	if (isset($_POST['form_sent']))
	{
		...
		if (empty($errors))
		{
			...
			$mail_tpl = forum_trim(file_get_contents(
				language()->path[user()->language] . '/mail_templates/form_email.tpl'));
...
else if (isset($_GET['report']))
{
	...
	// User pressed the cancel button
	if (isset($_POST['cancel']))
		redirect(link('post', $post_id), __('Cancel redirect'));
	...
	if (isset($_POST['form_sent']))
	{
		...
		if (empty($errors)) {
			...
			redirect(link('post', $post_id), __('Report redirect', 'misc'));
		}
	}
...
else if (isset($_GET['subscribe']))
{
	...
	redirect(link('topic', array($topic_id, sef_friendly($subject))),
...
else if (isset($_GET['unsubscribe']))
{
	...
	redirect(link('topic', array($topic_id, sef_friendly($subject))), __('Unsubscribe redirect', 'misc'));
}
...
else if (isset($_GET['forum_subscribe']))
{
	...
	redirect(link('forum', array($forum_id, sef_friendly($forum_name))),
		__('Subscribe redirect', 'misc'));
}
